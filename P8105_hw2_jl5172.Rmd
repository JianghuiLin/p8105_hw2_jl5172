...
entrancenoven<-filter(nycdistinct,entry=="YES" & vending=="NO") # Use filter to select stations which statisfy conditions that have no vending but allow entrance
...
as.integer(round(water$`Sports Balls`)) #round the values in 'Sports Balls' and transform this column into integers
```
...
# Based on my observations,'Date'and 'Month' are considered the most important  variables in the datasets. The amount of precipitation changed as the date goes by and we can see a pattern that precipitation amount is relatively high during summer time and low during winter time.
...
No, there are `r uniquestates` states which have been represented. 
...
median(brfss_2002$Excellent,na.rm=TRUE) # Calulate the median of excellent reponse in 2002 by using built-in function
```
In 2002, what is the median of the “Excellent” response value? 
# Median is `r median(brfss_2002$Excellent,na.rm=TRUE)`.
